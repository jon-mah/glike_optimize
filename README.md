...
Find in [paper scripts](./paper/) codes for reproducing the figures (including Fastsimcoal2 experiments) in the paper.
...
gLike provides some functionalities that may be helpful to some users.
...
Here `demo` is a `glike.Demo` object, and the returned `demography` is an `msprime.Demography` object.
Because in the general case, a non-sparse migration matrix cannot be represented as a set of admixtures and/or splits, the migrations in the msprime demography are constructed purely by `msprime.Demography.add_mass_migration()`.

The coalescence distribution of a demography is often of interest to researchers. 
Therefore, gLike provides several tools to conveniently study coalescent event times.
The basic function is `glike.get_coals()`, which computes the coalescent times of a genealogical tree.
...
More often the coalescence distribution of a list of trees is directly useful.
    ...
    coals = glike.get_coals_trees(trees)

Where `tree` is a list of `msprime.Tree` objects that have the same number of samples. The returned object is an 2D array of shape `len(trees) x trees[0].num_samples()-1`, where each row is the coalescent times of a genealogical tree in ascending order.

The expected coalescence distribution of a demography may be approximated by simulation.

    coals = glike.get_coals_demo(demo, samples_msprime, sims)

Where `demo` is a `glike.Demo` object, `samples_msprime` is a dict that is directly used by `msprime.sim_ancestry()`, and `sims` is the number of simulations. This is a simple wrapper function around existing utilities, defined as:
...
The coalescence distributions can be visualized conveniently:

    glike.plot_coalescent_distribution(ax, list_of_coals, names)

Where `ax` is an `matplotlib.axes._axes.Axes` object on which to plot the data. 
`list_of_coals` is a list of 2D arrays, each may be generated by `glike.get_coals_trees` or `glike.get_coals_demo`.
`names` is a list of the names of the demographies to be mentioned in the legend, it must have the same length as `list_of_coals`.
See [tutorial](./tutorial.md) for some simple use cases of this function.
